...
for subid, sesid in tqdm(subject_sessions):
    lesion_data['subid'].append(subid)
    ...
    lesion_data['total_count'] = lesion_stats.Num_Lesions
    lesion_data['total_volume'] = lesion_stats.Lesion_Volume
    ...
    for region, row in annotated_lesion_stats.iterrows():
        lesion_data[f"{region.lower()}_count"] = row.Num_Lesions
        lesion_data[f"{region.lower()}_volume"] = row.Lesion_Volume
