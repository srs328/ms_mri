...
# %%
def load_choroid(root, sub, ses):
    file = os.path.join(root, f"sub{sub}-{ses}", "choroid.nii.gz")
...
for i, row in tqdm(subject_sessions.iterrows(), total=len(subject_sessions)):
    ...
    ses = row['ses']
    try:
        choroid = load_function(dataproc_root, sub, ses)
        choroid_centroid = ndimage.center_of_mass(choroid)
