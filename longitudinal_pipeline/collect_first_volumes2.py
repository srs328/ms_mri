...
for subid, sessions in tqdm(subject_sessions.items(), total=len(subject_sessions)):
    ...
    for sesid in sessions:
        ...
        try:
            ...
            logger.warning(f"Missing something for {subject} in session {sesid}")
            logger.error(e)
            continue_subject = True
            break
        
        left_row  = {**base, "thalamus": left_vol}
        right_row = {**base, "thalamus": right_vol}
        full_row  = {**base, "thalamus": left_vol + right_vol}
