...
def main(argv=None):
    ...
    try:
        # args.contrastsynth = True  # SRS note: forces contrast synthesis on T1 inputs (passing -t1 to hipsthomas.sh)
        # args.algorithm = 'v2' by default, but can be set to 'v0' for legacy purposes (passing -a v0 to hipsthomas.sh)
        # args.smallCrop = False by default, but can be set to True to use a smaller crop region (passing -sc to hipsthomas.sh)
        # args.template and args.mask might not be supported by hiptsthomas.sh currently
