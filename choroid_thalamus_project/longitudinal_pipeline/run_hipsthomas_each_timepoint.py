...
# * Add a check to see if hipsthomas actually finished running.
# *   Ie don't just see that "left" exists, do a more robust check. Eg that the folder is greater than a certain size, or something even better
#!   Will definitely need to do this because I interrupted it a few times and didn't delete the partial directories
...
subjects.sort()
# with open(
#     "/home/srs-9/Projects/ms_mri/choroid_thalamus_project/longitudinal_pipeline/subjects_to_process.txt",
#     "r",
# ) as f:
#     subids = [line.strip() for line in f.readlines()]
subids = [re.search(r"(\d{4})", subject)[1] for subject in subjects]


hipsthomas_script = "/home/srs-9/Projects/ms_mri/choroid_thalamus_project/longitudinal_pipeline/runHipsThomas.sh"

now = datetime.now()
log_filename = now.strftime("run_hipsthomas-%Y%m%d_T%H%M%S.log")
logger.remove()
...
for subid in tqdm(subids, total=len(subids)):
    ...
    if (
        not (ses1_dir / "left" / "nucleiVols.txt").exists()
        or not (ses1_dir / "right" / "nucleiVols.txt").exists()
    ):
        scan1 = list(
            work_dir.glob(f"{subject}_input0000*_mniWarped-WarpedToTemplate.nii.gz")
        )[0]
    ...
    if (
        not (ses2_dir / "left" / "nucleiVols.txt").exists()
        or not (ses2_dir / "right" / "nucleiVols.txt").exists()
    ):
        scan2 = list(
            work_dir.glob(f"{subject}_input0001*_mniWarped-WarpedToTemplate.nii.gz")
        )[0]
