...
ensemble_root=/home/srs-9/Projects/ms_mri/training_work_dirs/cp_work_dir_choroid1/ensemble_output/3Tpioneer_bids
save_dir=/home/srs-9/Projects/ms_mri/monai_analysis/choroid1/screenshots

data_dir="$data_root/$sub/$ses"
ensemble_dir="$ensemble_root/$sub/$ses"

viewports=("sagittal" "coronal" "axial")
slice_coords=(132 121 170)

for i in {0..2}; do

    freeview -v "$data_dir/flair.nii.gz" \
        -v "$data_dir/choroid_t1_flair.nii.gz":colormap=heat \
    ...
    freeview -v "$data_dir/flair.nii.gz" \
        -v "$ensemble_dir/t1_flair_ensemble.nii.gz":colormap=heat \
        --slice ${slice_coords[@]} --viewport ${viewports[i]} \
